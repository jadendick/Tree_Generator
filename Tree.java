...
public class Tree
{
    ...
    public Tree(Point centerPoint) { this(centerPoint, System.currentTimeMillis()); } // Use a random seed
    public Tree(Point centerPoint, long seed)
    {
    ...
    {
        trunk = new Branch(centerPoint, new Point(centerPoint.x, centerPoint.y-2*maxBranchLength), 90, maxBranchLength, 0);  // Make the trunk
    ...
    {
        if(b.height >= maxHeight || b.connections.size() >= maxBranchConnections || rng.nextInt(100) > density)   // If branch has reached a limit, don't add more branches
            return;
...
class Branch
{
    ArrayList<Branch> connections = new ArrayList<Branch>();  // try without some default stuff
    Point start, end;
